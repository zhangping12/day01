...
junit要注意的细节：
   ...
   3、@Test测试的方法不能是static修饰和不能带有形参。
   4、如果测试一个方法的时候需要准备测试的环境或者是清理测试环境，那么可以@Before、@After、@BeforeClass、@AfterClass这四个注解。
   @Before、@After是在每个测试方法测试的时候都会调用一次，@BeforeClass、@AfterClass是在所有测试方法测试之前与测试之后调用一次而已。
